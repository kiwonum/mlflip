...
KERNEL(pts) template<class T, class S> void knPdataInvMultScalar(ParticleDataImpl<T>& me, const S& other) { me[idx] = safeDivide(other, me[idx]); }
...
int knCountParticles(const ParticleDataImpl<int> &t, const int flag) { if(t[idx]&flag) { ++result; } }
PYTHON() int countParticles(const ParticleDataImpl<int> &t, const int flag) { return knCountParticles(t, flag); }

KERNEL(pts)
void knGetGridIdx(ParticleDataImpl<int> &gidx, const BasicParticleSystem &parts, const GridBase &grid)
{
	const Vec3i p = toVec3i(parts.getPos(idx));
	gidx[idx] = (grid.isInBounds(p)) ? grid.index(p) : -1;
}
PYTHON() void getGridIdx(ParticleDataImpl<int> &gidx, const BasicParticleSystem &parts, const GridBase &grid) { knGetGridIdx(gidx, parts, grid); }
